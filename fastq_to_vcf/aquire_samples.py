...
freallove=$(realpath /home/vdp5/data/bam/{}.bam)
...
java -jar -Xmx7g /gpfs/fs0/data/wraycompute/vdp5/bin/GenomeAnalysisTK.jar -T HaplotypeCaller -nct 4 -R $ref -I recal_reads.bam --emitRefConfidence GVCF -o /gpfs/fs0/data/wraycompute/vdp5/gvcf/$base.g.vcf
rm -rf $SCRATCH
...
for alpha in os.listdir(args.directory):
	samples.append(alpha.split('.')[0][:-2])



for alpha in os.listdir('/home/vdp5/data/gvcf/'):
	samp = alpha.split('.')[0]
	samples.remove(samp)
...
for theta in samples:
	newstr = 'srun --mem=8G bwa mem -M -t 4 -v 2 -A 2 -L 15 -U 9 -T 75 -k 19 -w 100 -d 100 -r 1.5 -c 10000 -B 4 -O 6 -E 1 {} {} {} | samtools view  -Sb - | samtools sort - -o /home/vdp5/data/bam/{}.bam'.format('/gpfs/fs0/data/wraycompute/vdp5/reference_data/PVP01.fasta',os.path.join(args.directory , theta + '_1.fastq.gz'), os.path.join(args.directory , theta + '_2.fastq.gz'), theta)
