...
CONFIG_PATH="/home/vdp5/data/scripts/merged_DRC.config"
CMD_PATH="/gpfs/fs0/data/wraycompute/vdp5/src/MaSuRCA-3.2.2/bin/masurca"
...
export PATH="/gpfs/fs0/data/wraycompute/vdp5/src/MaSuRCA-3.2.2/bin:/gpfs/fs0/data/wraycompute/vdp5/src/MaSuRCA-3.2.2/bin/../CA/Linux-amd64/bin:$PATH"
save PATH
export PERL5LIB=/gpfs/fs0/data/wraycompute/vdp5/src/MaSuRCA-3.2.2/bin/../lib/perl${PERL5LIB:+:$PERL5LIB}
save PERL5LIB
NUM_THREADS=4
save NUM_THREADS
log 'Processing pe library reads'
rm -rf meanAndStdevByPrefix.pe.txt
echo 'pe 149 3673' >> meanAndStdevByPrefix.pe.txt
rename_filter_fastq 'pe' <(exec expand_fastq '/gpfs/fs0/data/wraycompute/vdp5/variant_staging/merged.R1.fastq' | awk '{if(length($0>200)) print substr($0,1,200); else print $0;}') <(exec expand_fastq '/gpfs/fs0/data/wraycompute/vdp5/variant_staging/merged.R2.fastq' | awk '{if(length($0>200)) print substr($0,1,200); else print $0;}' ) > 'pe.renamed.fastq'
...
JF_SIZE=`ls -l *.fastq | awk '{n+=$5}END{s=int(n/50); if(s>2200000000)printf "%.0f",s;else print "2200000000";}'`
save JF_SIZE
perl -e '{if(int('$JF_SIZE')>2200000000){print "WARNING: JF_SIZE set too low, increasing JF_SIZE to at least '$JF_SIZE', this automatic increase may be not enough!\n"}}'
log Creating mer database for Quorum.
quorum_create_database -t 4 -s $JF_SIZE -b 7 -m 24 -q $((MIN_Q_CHAR + 5)) -o quorum_mer_db.jf.tmp pe.renamed.fastq && mv quorum_mer_db.jf.tmp quorum_mer_db.jf
if [ 0 != 0 ]; then
...
quorum_error_correct_reads  -q $((MIN_Q_CHAR + 40)) --contaminant=/gpfs/fs0/data/wraycompute/vdp5/src/MaSuRCA-3.2.2/bin/../share/adapter.jf -m 1 -s 1 -g 1 -a 3 -t 4 -w 10 -e 3 -M  quorum_mer_db.jf pe.renamed.fastq --no-discard -o pe.cor --verbose 1>quorum.err 2>&1 || {
  mv pe.cor.fa pe.cor.fa.failed && fail Error correction of PE reads failed. Check pe.cor.log.
}


log Estimating genome size.
jellyfish count -m 31 -t 4 -C -s $JF_SIZE -o k_u_hash_0 pe.cor.fa
export ESTIMATED_GENOME_SIZE=`jellyfish histo -t 4 -h 1 k_u_hash_0 | tail -n 1 |awk '{print $2}'`
save ESTIMATED_GENOME_SIZE
echo "Estimated genome size: $ESTIMATED_GENOME_SIZE"

log Creating k-unitigs with k=$KMER
create_k_unitigs_large_k -c $(($KMER-1)) -t 4 -m $KMER -n $(($ESTIMATED_GENOME_SIZE*2)) -l $KMER -f `perl -e 'print 1/'$KMER'/1e5'` pe.cor.fa  | grep --text -v '^>' | perl -ane '{$seq=$F[0]; $F[0]=~tr/ACTGactg/TGACtgac/;$revseq=reverse($F[0]); $h{($seq ge $revseq)?$seq:$revseq}=1;}END{$n=0;foreach $k(keys %h){print ">",$n++," length:",length($k),"\n$k\n"}}' > guillaumeKUnitigsAtLeast32bases_all.fasta.tmp && mv guillaumeKUnitigsAtLeast32bases_all.fasta.tmp guillaumeKUnitigsAtLeast32bases_all.fasta
...
create_k_unitigs_large_k -c $(($KMER_J-1)) -t 4 -m $KMER_J -n $(($ESTIMATED_GENOME_SIZE*2)) -l $KMER_J -f `perl -e 'print 1/'$KMER_J'/1e5'` pe.cor.fa  | grep --text -v '^>' | perl -ane '{$seq=$F[0]; $F[0]=~tr/ACTGactg/TGACtgac/;$revseq=reverse($F[0]); $h{($seq ge $revseq)?$seq:$revseq}=1;}END{$n=0;foreach $k(keys %h){print ">",$n++," length:",length($k),"\n$k\n"}}' > guillaumeKUnitigsAtLeast32bases_all.jump.fasta.tmp && mv guillaumeKUnitigsAtLeast32bases_all.jump.fasta.tmp guillaumeKUnitigsAtLeast32bases_all.jump.fasta 
fi


log 'Computing super reads from PE '
rm -rf work1
createSuperReadsForDirectory.perl -l $KMER -mean-and-stdev-by-prefix-file meanAndStdevByPrefix.pe.txt -kunitigsfile guillaumeKUnitigsAtLeast32bases_all.fasta -t 4 -mikedebug work1 pe.cor.fa 1> super1.err 2>&1
...
grep --text -A 1 '^>pe' pe.linking.fa | grep --text -v '^\-\-' | sample_mate_pairs.pl $MAX_LINKING_MATES $NUM_LINKING_MATES 1 > pe.tmp && error_corrected2frg pe 149 3673 2000000000 pe.tmp > pe.linking.frg.tmp && rm pe.tmp && mv pe.linking.frg.tmp pe.linking.frg
echo "Using linking mates"
...
ovlMerThreshold=`jellyfish histo -t 4 k_u_hash_0 | awk '{thresh=75;if($1>1) {dist+=$2;if(dist>int("'$ESTIMATED_GENOME_SIZE'")*0.98&&flag==0){if($1>thresh) thresh=$1;flag=1}}}END{print thresh}'`
echo ovlMerThreshold=$ovlMerThreshold

runCA ovlMerThreshold=$ovlMerThreshold gkpFixInsertSizes=0 cgwErrorRate=0.15 jellyfishHashSize=$JF_SIZE ovlRefBlockSize=$ovlRefBlockSize ovlHashBlockSize=$ovlHashBlockSize ovlCorrBatchSize=$ovlCorrBatchSize stopAfter=consensusAfterUnitigger unitigger=bog -p genome -d CA merylThreads=4 frgCorrThreads=1 frgCorrConcurrency=4 cnsConcurrency=2 ovlCorrConcurrency=4 ovlConcurrency=4 ovlThreads=1 ovlMemory=4GB doFragmentCorrection=0 doOverlapBasedTrimming=0 doExtendClearRanges=0 ovlMerSize=30 superReadSequences_shr.frg   pe.linking.frg    1> runCA1.out 2>&1
...
( cd CA && tigStore -g genome.gkpStore -t genome.tigStore 2 -d layout -U | tr -d '-' | awk 'BEGIN{print ">unique unitigs"}{if($1 == "cns"){seq=$2}else if($1 == "data.unitig_coverage_stat" && $2>=5){print seq"N"}}' | jellyfish count -L 2 -C -m 30 -s $ESTIMATED_GENOME_SIZE -t 4 -o unitig_mers /dev/fd/0 &&  cat <(overlapStore -b 1 -e $NUM_SUPER_READS -d genome.ovlStore  | awk '{if($1<'$NUM_SUPER_READS' && $2<'$NUM_SUPER_READS') print $0}'|filter_overlap_file -t 4 <(gatekeeper -dumpfastaseq genome.gkpStore ) unitig_mers /dev/fd/0) <(overlapStore -d genome.ovlStore | awk '{if($1>='$NUM_SUPER_READS' || $2>='$NUM_SUPER_READS') print $1" "$2" "$3" "$4" "$5" "$6" "$7}')  |convertOverlap -b -ovl > overlaps.ovb &&  rm -rf 4-unitigger 5-consensus genome.tigStore && mkdir ovlStoreBackup && mv genome.ovlStore ovlStoreBackup && overlapStore -c genome.ovlStore -M 4096 -t 4 -g genome.gkpStore overlaps.ovb 1>overlapstore.err 2>&1 && rm overlaps.ovb && touch overlapFilter.success
)
fi
...
recompute_astat_superreads.sh genome CA $PE_AVG_READ_LENGTH work1/readPlacementsInSuperReads.final.read.superRead.offset.ori.txt
runCA cgwErrorRate=0.15 unitigger=bog -p genome -d CA cnsConcurrency=2 computeInsertSize=0 ovlMemory=4GB doFragmentCorrection=0 doOverlapBasedTrimming=0 doExtendClearRanges=0 ovlMerSize=30 1>runCA3.out 2>&1
...
log 'Gap closing'
closeGapsLocally.perl --max-reads-in-memory 1000000000 -s 2200000000 --Celera-terminator-directory CA/9-terminator --reads-file 'pe.renamed.fastq' --output-directory CA/10-gapclose --min-kmer-len 17 --max-kmer-len $(($PE_AVG_READ_LENGTH-5)) --num-threads 4 --contig-length-for-joining $(($PE_AVG_READ_LENGTH-1)) --contig-length-for-fishing 200 --reduce-read-set-kmer-size 21 1>gapClose.err 2>&1
